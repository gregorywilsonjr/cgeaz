...
| Change alert, before it was applied | The first draft of the alert query filtered on `ResourceId`, a legacy `AzureActivity` column that many operations leave empty, so the tripwire could have watched and reported nothing | Medium | The query uses `_ResourceId`, and it was run against `law-grc-sandbox` before the pull request opened ([PR #16](https://github.com/gregorywilsonjr/cgeaz/pull/16)) | The pre-merge run returned 7 changes in 3 hours from 1 caller; the alert had fired four times by 2026-09-22 |
